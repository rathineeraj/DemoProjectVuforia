...
// ----------------------------------------------------------------------------
static const char* const kAccessKey = "5a0794d1e5a75b966856d5c06e8c2645b8392125";
static const char* const kSecretKey = "77c578d8da3446f9e5c9a923dbb760b7b248883e";
...
-(void)showAlertWithMessage:(NSString*)message {
    //  Call the UIAlert on the main thread to avoid undesired behaviors
    dispatch_async( dispatch_get_main_queue(), ^{
        UIAlertView *anAlertView = [[[UIAlertView alloc] initWithTitle:nil
                                                                   message:message
                                                                  delegate:self
                                                         cancelButtonTitle:@"OK"
                                                         otherButtonTitles:nil] autorelease];
        ...
        for (int i = 0; i < finder->getResultCount(); ++i)
        {
            ...
            if (result->getTrackingRating() > 0)
            {
                ...
                if (newTrackable != 0)
                {
                    ...
                    [self showAlertWithMessage:[NSString stringWithFormat:@"Successfully created new trackable '%s' with rating '%d'.",
                                                newTrackable->getName(), result->getTrackingRating()]];
                }
                else
                {
